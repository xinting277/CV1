...
function txtCanvasLayer() {
    ...
    img.onload = function () {
        ...
        $(canvasArea).on('mousedown', function (e) {
            txtCanvasDown(e, this, div)
        })
        $(canvasArea).on('mousemove', function (e) {
            txtCanvasMove(e, this)
        });
        $(canvasArea).on('mouseup', function () {
            txtCanvasUp(this, div)
        });
        $(canvasArea).on('mouseout', function () {
            txtCanvasUp(this, div)
        });

        $(canvasArea).on('touchstart', function (e) {
            txtCanvasDown(e, this, div)
        })
        $(canvasArea).on('touchmove', function (e) {
            txtCanvasMove(e, this)
        });
        $(canvasArea).on('touchend', function () {
            txtCanvasUp(this, div)
        });
...
function txtCanvasDown(event, canvas, div) {
    ...
    var list = {
        X: Number(txtCanvas.Down.X),
        Y: Number(txtCanvas.Down.Y)
    };
...
function txtCanvasMove(event, canvas) {
    ...
    if (txtCanvas.Drag && !MainObj.isPinch) {
        ...
        //將畫線的座標都存入txtCanvas.Line裡面
        var list = {
            X: Number(txtCanvas.Move.X),
            Y: Number(txtCanvas.Move.Y)
        };
        txtCanvas.Line.X.push(list.X);
        txtCanvas.Line.Y.push(list.Y);
...
function txtCanvasUp(canvas, div) {
    if (txtCanvas.Drag) {
        ...
        for (var i = 1; i < txtCanvas.Line.X.length; i++) {
            ...
            list.push({
                X: txtCanvas.Line.X[i] / MainObj.Scale,
                Y: txtCanvas.Line.Y[i] / MainObj.Scale,
                id: canvas.id
            });
        }
